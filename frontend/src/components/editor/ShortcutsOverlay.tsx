  ...
  return (
    ...
    >
      <div
        ref={dialogRef}
        tabIndex={-1}
        className="w-full max-w-sm rounded-xl border bg-white shadow-xl outline-none dark:border-slate-700 dark:bg-slate-800 dark:shadow-slate-900/60"
      >
        {/* Header */}
        ...
        {/* Shortcut list */}
        <ul className="divide-y px-5 dark:divide-slate-700">
          {SHORTCUTS.map(({ keys, description }) => (
            <li key={keys} className="flex items-center justify-between py-3">
              ...
              <kbd className="rounded border bg-gray-100 px-2 py-0.5 font-mono text-xs text-gray-700 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200">
                {keys}
              </kbd>
            </li>
          ))}
        ...
        <div className="px-5 pb-4 pt-2 text-xs text-gray-400 dark:text-slate-400">
